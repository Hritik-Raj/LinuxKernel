...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
	    ...
	    // if (mod_count == 0) /* filesystem image */ {
	    // 	fs_img = (struct bootblock *) mod->mod_start;
		// printf("Filesystem has %d inodes\n", fs_img->nr_inodes);
	    // }
    ...
    keyboard_init();            // Calls the init function for keyboard
    fs_init((module_t*)mbi->mods_addr);
    init_paging();
    // clear();
    uint8_t* shell_val = (uint8_t*)"shell";
    execute(shell_val);
    // execute("ls");


    // rtc_init();                 // Calls the init function for rtc
